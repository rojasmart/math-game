...
export default function SubtractionGameLevelThree() {
  ...
  // Generate new numbers for triple subtraction
  const generateNumbers = () => {
    let range = 20;
    if (userSettings?.difficulty === "easy") range = 10;
    if (userSettings?.difficulty === "hard") range = 50;

    // Generate numbers ensuring each subtraction is possible
    const missing = Math.floor(Math.random() * range) + 1;
    const n3 = Math.floor(Math.random() * range) + missing;
    const n2 = Math.floor(Math.random() * range) + n3;
    const n1 = Math.floor(Math.random() * range) + n2;
    const finalResult = n1 - n2 - n3 - missing;
  ...
  // ...existing useEffect hooks for mounted, timer, and keyboard input...
  useEffect(() => {
    if (!mounted) {
      setMounted(true);
  ...
  const checkAnswer = () => {
    ...
    const correctAnswer = num1 - num2 - num3 - result; // What number, when subtracted, gives us the result

    const responseTime = (Date.now() - questionStartTime) / 1000;
    setTotalQuestions((prev) => prev + 1);

    if (userNum === correctAnswer) {
      // Reward faster responses with more points
      let timeBonus = 0;
      if (responseTime <= 1) {
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-8">
      <GameHeader title="Level 3" showPlayAgain={!isActive} onPlayAgain={restartGame} returnPath="/games/subtraction" />
