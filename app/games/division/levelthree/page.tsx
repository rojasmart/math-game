...
export default function DivisionGameLevelThree() {
  ...
  const generateProblem = () => {
    // For Level 3, we need to find the missing third divisor: a ÷ b ÷ ? = result
    // Start with small whole numbers for the final result
    let finalResultMax = 5;
    if (userSettings?.difficulty === "medium") finalResultMax = 8;
    if (userSettings?.difficulty === "hard") finalResultMax = 10;

    const finalResult = Math.floor(Math.random() * finalResultMax) + 1;

    // Generate second divisor (visible to the user)
    let maxSecondDivisor = 5;
    if (userSettings?.difficulty === "medium") maxSecondDivisor = 7;
    if (userSettings?.difficulty === "hard") maxSecondDivisor = 9;

    const secondDivisor = Math.floor(Math.random() * (maxSecondDivisor - 1)) + 2; // Avoid division by 1

    // Generate third divisor (will be the answer)
    let maxThirdDivisor = 5;
    if (userSettings?.difficulty === "medium") maxThirdDivisor = 7;
    if (userSettings?.difficulty === "hard") maxThirdDivisor = 9;
  ...
  // Check answer
  const checkAnswer = () => {
    const correctAnswer = num1 / num2 / result;
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-8">
      <GameHeader title="Level 3" showPlayAgain={!isActive} onPlayAgain={restartGame} returnPath="/games/division" />
