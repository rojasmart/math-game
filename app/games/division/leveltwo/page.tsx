...
export default function DivisionGameLevelTwo() {
  ...
  const generateProblem = () => {
    // Choose a divisor range based on difficulty
    let maxDivisor = 10;
    ...
    // Generate a divisor (second number)
    const divisor = Math.floor(Math.random() * (maxDivisor - 1)) + 2; // Avoid division by 1

    // Generate a quotient (result) - small whole numbers for easy mental math
    let maxQuotient = 10;
    if (userSettings?.difficulty === "easy") maxQuotient = 6;
    if (userSettings?.difficulty === "hard") maxQuotient = 15;

    const quotient = Math.floor(Math.random() * maxQuotient) + 1;

    // Calculate the dividend (first number, which will be the answer)
  ...
  // Check answer
  const checkAnswer = () => {
    const correctAnswer = num1 * result;
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-8">
      <GameHeader title="Level 2" showPlayAgain={!isActive} onPlayAgain={restartGame} returnPath="/games/division" />
