...
export default function MultiplicationGameLevelThree() {
  ...
  const generateNumbers = () => {
    let range = 10;
    if (userSettings?.difficulty === "easy") range = 5;
    if (userSettings?.difficulty === "hard") range = 12;

    const n3 = Math.floor(Math.random() * range) + 1;
    const n2 = Math.floor(Math.random() * range) + 1;
    const n1 = Math.floor(Math.random() * range) + 1;
    const resultVal = n1 * n2 * n3;
  ...
  const checkAnswer = () => {
    ...
    const missingNumber = result / (num1 * num2);

    setTotalQuestions((prev) => prev + 1);

    if (userNum === missingNumber) {
      let timeBonus = 0;
      if (responseTime <= 2) {
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-8">
      <GameHeader title="Level 3" showPlayAgain={!isActive} onPlayAgain={restartGame} returnPath="/games/multiplication" />
