...
export default function MultiplicationGameLevelFour() {
  ...
  const generateProblem = () => {
    let range = 5;
    if (userSettings?.difficulty === "medium") range = 7;
    if (userSettings?.difficulty === "hard") range = 9;

    // Generate smaller numbers to prevent the result from being too large
    const n1 = Math.floor(Math.random() * (range - 1)) + 1;
    const n2 = Math.floor(Math.random() * (range - 1)) + 1;
    const n3 = Math.floor(Math.random() * (range - 1)) + 1;
    const n4 = Math.floor(Math.random() * (range - 1)) + 1;
    const n5 = Math.floor(Math.random() * (range - 1)) + 1;

    setNum1(n1);
    setNum2(n2);
    setNum3(n3);
    setNum4(n4);
    setResult(n1 * n2 * n3 * n4 * n5);
    setUserAnswer("");
    setFeedback("");
  };
  ...
  // Check answer
  const checkAnswer = () => {
    const correctAnswer = result / (num1 * num2 * num3 * num4);
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-8">
      <GameHeader title="Level 4" showPlayAgain={!isActive} onPlayAgain={restartGame} returnPath="/games/multiplication" />
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="flex-1">
                ...
                <div className="flex justify-center md:justify-start items-center mb-8 text-5xl font-bold flex-wrap">
                  ...
                  <span className="text-gray-700 mx-3">×</span>
                  <span className="text-blue-600">{num4}</span>
                  <span className="text-gray-700 mx-3">×</span>
