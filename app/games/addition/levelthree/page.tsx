...
export default function AdditionGameLevelThree() {
  const { userSettings, addPoints } = useUser();
  const [mounted, setMounted] = useState(false);

  const getDefaultTime = () => {
    if (userSettings.difficulty === "easy") return 15;
    if (userSettings.difficulty === "medium") return 10;
    return 7; // hard
  };
  ...
  const generateProblem = () => {
    ...
    const finalResult = firstNum + secondNum + thirdNum + Math.floor(Math.random() * range) + 1;
  ...
  // Check answer
  const checkAnswer = () => {
    const correctAnswer = result - (num1 + num2 + num3);
